...
module Crossword
  ...
  class Drawer
    ...
    def clues(current)
      across_point = Point(ACROSS_LEFT, MARGIN * 2)
      down_point   = Point(DOWN_LEFT, MARGIN * 2)
    ...
    # Render the clue list off screen first if it's the list with the current
    # clue, then redraw it where asked, potentially not from the start if the
    # current clue wouldn't be displayed.
